...
import { editAnswerComment, fetchAnswerComments } from '../helpers/comment-helpers'
import {
  createQuestion,
  fetchQuestions,
  getQuestionBySlug
...
describe('Edit Answer Comment Route', () => {
  ...
  it('should return 400 when content is missing', async () => {
    const httpResponse = await editAnswerComment(app, authToken, commentId, {})
  ...
  it('should return 404 when trying to edit non-existent comment', async () => {
    const fakeCommentId = anAnswer().build().id
    const httpResponse = await editAnswerComment(app, authToken, fakeCommentId, {
  ...
  it('should return 403 when trying to edit comment as non-author', async () => {
    const httpResponse = await editAnswerComment(app, otherUserToken, commentId, {
      content: 'Updated content'
    })
  ...
  it('should return 200 and update the comment on successful edit', async () => {
