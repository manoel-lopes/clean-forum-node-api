...
describe('EditCommentController', () => {
  let sut: EditCommentController
  let editCommentUseCase: UseCase
  const httpRequest = {
    params: {
  ...
  beforeEach(() => {
    editCommentUseCase = new UseCaseStub()
    sut = new EditCommentController(editCommentUseCase)
  ...
  it('should return 404 code and a not found error response if the comment is not found', async () => {
    vi.spyOn(editCommentUseCase, 'execute').mockRejectedValue(
