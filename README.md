...
- **Singleton**: Ensures a class has only one instance and provides a global point of access to it. Used in `EmailQueueProducer` to maintain a single shared queue instance across the application, preventing multiple queue connections and ensuring consistent job management.
- **Pub/Sub (Publisher-Subscriber)**: Implements asynchronous message-based communication where producers publish messages to queues and consumers subscribe to process them independently. Used in the email queue system with `EmailQueueProducer` publishing email jobs and `EmailQueueConsumer` processing them asynchronously with retry logic and error handling.
